  ...
  return (
    <Link
      ...
      style={{
        ...
        borderTop: `3px solid ${hasSession ? (isMarked ? "var(--os-success)" : "var(--os-warning)") : "var(--os-text-disabled)"}`,
    ...
    >
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "0.75rem" }}>
        <div style={{ minWidth: 0 }}>
          <p style={{ margin: "0 0 0.15rem", fontSize: "0.9375rem", fontWeight: 600, color: "var(--os-text-primary)" }}>{cls.name}</p>
          <p style={{ margin: 0, fontSize: "0.75rem", color: "var(--os-text-tertiary)" }}>{cls.grade_name}</p>
        </div>
        {hasSession ? (
          isMarked ? (
            <CheckmarkFilled size={18} style={{ fill: "var(--os-success)", flexShrink: 0 }} />
          ) : (
            <WarningFilled size={18} style={{ fill: "var(--os-warning)", flexShrink: 0 }} />
          )
        ) : (
          <EventSchedule size={18} style={{ fill: "var(--os-text-disabled)", flexShrink: 0 }} />
        )}
      </div>
      ...
      {hasSession ? (
        <>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.75rem", marginBottom: "0.35rem" }}>
            <span style={{ color: "var(--os-text-secondary)" }}>{isMarked ? "Marked today" : "Pending today"}</span>
            <span style={{ fontWeight: 600, color: "var(--os-text-primary)" }}>
              {session.marked_count} / {session.enrolled_count}
            </span>
          </div>
          <div style={{ height: "5px", background: "var(--os-border-subtle)", borderRadius: "3px" }}>
            <div
              style={{
                width: `${session.enrolled_count > 0 ? Math.round((session.marked_count / session.enrolled_count) * 100) : 0}%`,
                height: "100%",
                background: isMarked ? "var(--os-success)" : "var(--os-warning)",
                borderRadius: "3px",
              }}
            />
          </div>
        ...
        <p style={{ margin: 0, fontSize: "0.75rem", color: "var(--os-text-tertiary)" }}>No session today</p>
      )}
    </Link>
  );
}
  ...
  return (
    <div style={{ padding: "1.25rem 1.5rem" }}>
      <div style={{ marginBottom: "1rem" }}>
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.75rem", marginBottom: "0.35rem" }}>
          <span style={{ color: "var(--os-text-secondary)" }}>Marked today</span>
          <span style={{ fontWeight: 600, color: "var(--os-text-primary)" }}>
            {marked} / {total}
          </span>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(100px, 1fr))", gap: "0.75rem" }}>
        {stats.map((s) => (
          <div key={s.label} style={{ textAlign: "center", padding: "0.75rem", border: "1px solid var(--os-border-subtle)" }}>
            <p style={{ margin: "0 0 0.25rem", fontSize: "1.25rem", fontWeight: 600, color: s.color }}>{s.value}</p>
            <p style={{ margin: 0, fontSize: "0.75rem", color: "var(--os-text-tertiary)" }}>{s.label}</p>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    ...
    >
      {Array.from({ length: 4 }).map((_, i) => (
        <div key={i} style={{ border: "1px solid var(--os-border-subtle)", padding: "1rem 1.125rem" }}>
