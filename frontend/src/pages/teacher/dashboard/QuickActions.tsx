        ...
        {actions.map(({ label, desc, path, Icon }) => (
          <Link key={label} to={path} style={{ textDecoration: "none" }}>
            <div className="os-quick-action">
              <Icon size={20} style={{ fill: "var(--os-accent)", flexShrink: 0 }} />
              <div style={{ flex: 1 }}>
                <p style={{ margin: "0 0 0.1rem", fontWeight: 600, fontSize: "0.8125rem", color: "var(--os-text-primary)" }}>{label}</p>
                <p style={{ margin: 0, fontSize: "0.75rem", color: "var(--os-text-secondary)" }}>{desc}</p>
              </div>
              <ArrowRight size={14} style={{ fill: "var(--os-text-tertiary)" }} />
            </div>
          </Link>
        ))}
      </div>
